...
create table if not exists skills (
  ...
  output_callbacks jsonb, -- OutputCallback[] | null — fires on every successful execution of this skill (any source), posting the result's table to every configured URL, see lib/outputCallback.ts. Superseded the old single-object output_callback column.
  ...
  account_split   jsonb, -- SkillAccountSplit | null — Cowork-only, opt-in: splits a multi-account manual/scheduled run into one execution per account instead of asking Cowork to switch accounts mid-task; the actual account switch is a manual step in Cowork's own browser, gated by mac-agent/agent.js, see lib/accountSplit.ts
